...
public class ReportMerger {
	...
	public void mergeReports(File reportDirectory) throws Throwable {
		...
		for (File report : existingReports) {
			...
			if (report.getName().equals(reportFileName)) {
				// rename all the image files (to give unique names) in report
				// directory and update report
				renameEmbededImages(report);

				// if we are on the first pass, copy the directory of the file
				// to use as basis for merge
	...
	/**
	 * Give unique names to embedded images to ensure they aren't lost during
	 * merge Update report file to reflect new image names
	 *
	 * @param reportFile
	 */
